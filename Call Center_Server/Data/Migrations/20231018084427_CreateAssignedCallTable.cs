using Microsoft.EntityFrameworkCore.Migrations;

#nullable disable

namespace Call_Center_Server.Data.Migrations
{
    /// <inheritdoc />
    public partial class CreateAssignedCallTable : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.CreateTable(
                name: "AssignedCallModel",
                columns: table => new
                {
                    Id = table.Column<int>(type: "int", nullable: false)
                        .Annotation("SqlServer:Identity", "1, 1"),
                    Title = table.Column<string>(type: "nvarchar(10)", maxLength: 10, nullable: false),
                    AssignedEmployeeFullName = table.Column<string>(type: "nvarchar(100)", maxLength: 100, nullable: false),
                    AssignedEmployeeEmail = table.Column<string>(type: "nvarchar(100)", maxLength: 100, nullable: false),
                    IsAssigned = table.Column<bool>(type: "bit", nullable: false),
                    CallModelId = table.Column<int>(type: "int", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_AssignedCallModel", x => x.Id);
                    table.ForeignKey(
                        name: "FK_AssignedCallModel_Calls_CallModelId",
                        column: x => x.CallModelId,
                        principalTable: "Calls",
                        principalColumn: "Id",
                        onDelete: ReferentialAction.Cascade);
                });

            migrationBuilder.CreateIndex(
                name: "IX_AssignedCallModel_CallModelId",
                table: "AssignedCallModel",
                column: "CallModelId");
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropTable(
                name: "AssignedCallModel");
        }
    }
}
